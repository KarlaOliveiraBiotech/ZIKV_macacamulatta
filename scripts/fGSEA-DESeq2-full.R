...
for(file in files){
  ...
  res <- df %>% dplyr::select(Ensembl_id, log2FoldChange, pvalue, padj) 
  names(res)[1] <- "SYMBOL"
  
  res_uniq <- res[!duplicated(res[, "SYMBOL"]), ]
  
  ranks <- data.frame(SYMBOL = res_uniq$SYMBOL, Log2FC = res_uniq$log2FoldChange * -log10(res_uniq$pvalue))
  ...
  dirOut <- paste0("workflow/results/DESeq2/fGSEA/")
