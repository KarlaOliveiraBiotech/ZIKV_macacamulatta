...
raw_counts <- readr::read_tsv(file = "workflow/results/feature_counts/raw_count_matrix_ensemblid.tsv",
...
########## Prep lib_spec ##########
spl <- strsplit(x = lib_spec$Sample, split = "_") # Splits strings into list
spl <- sapply(spl, "[[",1) # Access a first element of each row in a list
spl <- as.numeric(spl) # Transforms substrings into numbers

lib_spec <- dplyr::mutate(lib_spec, spl) # Adds a column that will be used to order
lib_spec <- lib_spec[order(spl),] # Order by last column

lib_spec <- lib_spec %>% 
  dplyr::select(-c(5,6))

old_lib_spec <- lib_spec # Backup

lib_spec <- lib_spec %>% 
  tibble::column_to_rownames("Sample") # Uses samples as rownames
########## Prep lib_spec ##########



########## Decide factors ##########
lib_spec$Timepoint <- factor(lib_spec$Timepoint, levels = c("Dm14", "D1", "D3", "D5", 
                                                            "D7", "D10", "D14"))
lib_spec$Animal <- factor(lib_spec$Animal, levels = c("A", "B", "C", "D"))
lib_spec$Class <- factor(lib_spec$Class, levels = c("baseline", "ZIKV"))
########## Decide factors ##########
...
colnames(df_unsort) # To identify the pattern
cn <- base::gsub(pattern = "results/mapped/bowtie2/(.*)_S(.*)_L(.*)_good_clean.bam", 
                 replacement = "\\1", x = colnames(df_unsort))
colnames(df_unsort) <- cn

sample_names <- old_lib_spec$Sample

df <- data.frame(df_unsort, row.names = 1)
cn2 <- base::gsub(pattern = "X(.*)", replacement = "\\1", 
                  x = colnames(df))
cn2 <- base::gsub(patter = "(00.)", replacement = "\\00-", x = cn2)
colnames(df) <- cn2             
df <- df[, rownames(lib_spec)]
